...
ICON_VERTICAL_OFFSET = 14.0  # pt — spacing from bottom of legend cell to icon baseline
...
def _truncate_label(label: str, max_chars: int = 8) -> str:
    """Truncate a sign label to max_chars, appending an ellipsis if needed."""
...
def _draw_legend(
    ...
    for i, sign in enumerate(unique_signs):
        ...
        render_sign_to_canvas(c, sign, icon_x, y_icons + ICON_VERTICAL_OFFSET, size=20)

        label = _truncate_label(sign.label)
